...
DECLARE
    ...
    v_contact_type    TEXT;
BEGIN
    -- ── 1. Contact Info & Opening Balance
    ...
    -- ── 3. Statement Rows (grouped by voucher/arrival/sale)
    WITH raw_data AS (
        SELECT
            le.id,
            le.entry_date,
            ...
            a.total_value           AS arrival_gross_total,
            v.type                  AS v_type,
            v.voucher_no            AS v_voucher_no,
            v.narration             AS v_narration,
            v.invoice_id            AS v_invoice_id,
            s.bill_no               AS sale_bill_no,
            s.contact_bill_no       AS sale_contact_bill_no,
            s.total_amount_inc_tax  AS sale_gross_total,
    ...
    grouped_data AS (
        SELECT
            ...
            MAX(primary_ref_id)                                         AS primary_ref_id,
            MAX(v_invoice_total)                                        AS ref_invoice_total,
            MAX(v_invoice_no)                                           AS ref_invoice_no,
            MAX(v_arrival_total)                                        AS ref_arrival_total,
            MAX(v_arrival_no)                                           AS ref_arrival_no,
            CASE
            ...
            CASE
                ...
                WHEN (MAX(v_type) = 'receipt' OR (v_contact_type = 'buyer' AND SUM(credit) > 0)) AND MAX(sale_bill_no) IS NOT NULL THEN
                    ...
                    ' (Bill Amt: ₹' || TO_CHAR(MAX(sale_gross_total), 'FM9,99,99,999') || ')'
                ...
                WHEN (MAX(v_type) = 'payment' OR (v_contact_type != 'buyer' AND SUM(debit) > 0)) AND MAX(arrival_bill_no) IS NOT NULL THEN
                    ...
                    ' (Bill Amt: ₹' || TO_CHAR(MAX(arrival_gross_total), 'FM9,99,99,999') || ')'
        ...
        FROM (
            SELECT 
                *, 
                reference_id as primary_ref_id,
                sale_gross_total as v_invoice_total,
                COALESCE(sale_contact_bill_no, sale_bill_no::text) as v_invoice_no,
                arrival_gross_total as v_arrival_total,
                COALESCE(arrival_ref_no, arrival_bill_no::text) as v_arrival_no
            FROM raw_data
        ) t_raw
        GROUP BY group_id
    ),
            ...
            'products', (
                SELECT jsonb_agg(p) FROM (
                    ...
                    SELECT DISTINCT ON (si.id)
                        jsonb_build_object(
                            'name',      i.name,
                            'qty',       COALESCE(si.quantity, si.qty),
                    ...
                    SELECT DISTINCT ON (l2.id)
                        jsonb_build_object(
                            'name',      i1.name,
                            'qty',       COALESCE(l2.initial_qty, l2.weight, 0),
                            'unit',      l2.unit,
                    ...
                    WHERE a2.id = ANY(OuterQuery.arrival_ids)
                       ...
                       OR a2.id IN (SELECT arrival_id FROM mandi.vouchers WHERE id = ANY(OuterQuery.voucher_ids))
                ) t
            ),
            'charges', (
