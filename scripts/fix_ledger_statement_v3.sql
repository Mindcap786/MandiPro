    ...
    grouped_data AS (
        SELECT
            ...
            CASE
                ...
                -- Cash/Bank Receipts & Payments logic (Context aware)
                WHEN (MAX(v_type) IN ('receipt','payment') OR MAX(transaction_type) IN ('payment','receipt')) THEN
                    CASE 
                        WHEN v_contact_type = 'buyer' THEN 'RECEIPT'
                        ELSE 'PAYMENT'
                    END
                
                -- Fallbacks
                WHEN SUM(credit) > 0 THEN 
                    CASE WHEN v_contact_type = 'buyer' THEN 'RECEIPT' ELSE 'PURCHASE' END
                WHEN SUM(debit) > 0 THEN 
                    CASE WHEN v_contact_type = 'buyer' THEN 'SALE' ELSE 'PAYMENT' END
            ...
            'products', (
                SELECT jsonb_agg(p) FROM (
                    ...
                    SELECT DISTINCT ON (l2.id)
                        jsonb_build_object(
                            'name',      i1.name,
                            'qty',       COALESCE(l2.initial_qty, l2.weight),
                            'unit',      l2.unit,
