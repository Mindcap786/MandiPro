-- ============================================================================
-- MIGRATION 2: Complete Missing RLS Policies
-- Run in Supabase SQL Editor AFTER Migration 1
-- Safe: All DROP IF EXISTS + CREATE, idempotent
-- ============================================================================
...
-- ── STEP 1: core.profiles (special — self-read + org-read) ──
...
-- ── STEP 4: MANDI SCHEMA — Fill 6 missing tenant policies ──
...
-- ── STEP 6: Admin/Billing tables — RLS enabled, service_role only ──
...
-- app_plans: public read (users need to see plan options on billing page)
ALTER TABLE core.app_plans ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "plans_read_all" ON core.app_plans;
CREATE POLICY "plans_read_all" ON core.app_plans FOR SELECT USING (true);

-- system_alerts: tenants see their own
...
-- ── STEP 7: Grant helper functions to authenticated role ──
