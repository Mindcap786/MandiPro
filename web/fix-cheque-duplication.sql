-- Fix Cheque clearance double accounting for Arrivals

-- 1. First, cleanup the duplicate data created previously.
-- We must delete the duplicate "Voucher 32" (or similar) created by post_arrival_ledger
-- and we must fix the ledger entries so we only have one set.

-- We delete all payment vouchers that are system generated for an arrival, except the oldest one (which was the original cheque).
-- But let's build the correct post_arrival_ledger logic first.
...
BEGIN
    ...
    -- ONLY delete the purchase voucher, NOT the payment voucher (we will upsert it)
...
BEGIN
    ...
    ELSIF v_voucher.reference_id IS NOT NULL THEN
        -- If it was linked by reference_id (e.g. standard purchase but not an explicit lot advance link maybe?)
