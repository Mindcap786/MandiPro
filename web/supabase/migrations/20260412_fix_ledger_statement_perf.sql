-- ============================================================
-- PERFORMANCE: Fix get_ledger_statement dual-schema UNION ALL
-- Migration: 20260412_fix_ledger_statement_perf.sql
--
-- BEFORE: Every call scans BOTH mandi.ledger_entries AND
--         public.ledger_entries — 4 UNION ALL blocks = 8 scans
--         per ledger page load. ~400-1500ms per call.
--
-- AFTER:  Single-schema query against mandi.ledger_entries only.
--         Data migration (20260228_data_etl_sync.sql) already
--         moved all legacy public entries to mandi schema.
--         → ~30-80ms per call with the new indexes.
-- ============================================================
...
BEGIN
    -- ── 1. Opening Balance (uses idx_le_org_contact_date_cover) ─
    ...
    WITH raw_data AS (
        ...
        LEFT  JOIN mandi.lots     l ON (le.transaction_type = 'lot_purchase' OR le.reference_id = l.id)
                                    AND le.reference_id = l.id
            ...
            'charges', (
                SELECT jsonb_agg(c) FROM (
                    ...
                    FROM (
                        ...
                        UNION ALL
                        SELECT 'Misc',                misc_fee::numeric                   FROM mandi.sales WHERE id = OuterQuery.invoice_id OR id = ANY(OuterQuery.reference_ids)
                        UNION ALL
                        SELECT 'Hire',                hire_charges::numeric               FROM mandi.arrivals WHERE id = ANY(OuterQuery.arrival_ids)
                        UNION ALL
                        SELECT 'Labor',               hamali_expenses::numeric            FROM mandi.arrivals WHERE id = ANY(OuterQuery.arrival_ids)
                        UNION ALL
                        SELECT 'Other Exp',           other_expenses::numeric             FROM mandi.arrivals WHERE id = ANY(OuterQuery.arrival_ids)
    ...
    -- ── 4. Closing Balance (uses same index as opening) ─────────
    SELECT COALESCE(SUM(debit), 0) - COALESCE(SUM(credit), 0)
