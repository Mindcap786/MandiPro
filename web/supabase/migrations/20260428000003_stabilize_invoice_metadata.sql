-- V13+: Stabilize Invoice Metadata (Vehicle, Book, Lot)
-- This migration ensures lot_no is stored on sales and propagated correctly from sessions.

-- 1. Add missing lot_no column to mandi.sales
ALTER TABLE mandi.sales ADD COLUMN IF NOT EXISTS lot_no TEXT;

-- 2. Consolidate and Update confirm_sale_transaction RPC
-- We drop existing versions to ensure we have a single, clean signature.
-- Based on previous audit, there were versions with different parameter counts.
DROP FUNCTION IF EXISTS mandi.confirm_sale_transaction(uuid, uuid, date, text, numeric, jsonb, numeric, numeric, numeric, numeric, numeric, numeric, numeric, text, date, uuid, text, date, boolean, text, numeric, numeric, numeric, numeric, numeric, numeric, text, text, boolean, text, text);
...
-- 3. Update commit_mandi_session to propagate metadata
