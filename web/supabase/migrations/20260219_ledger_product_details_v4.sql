-- Migration: Debug Product Details Logic v4
-- Date: 2026-02-19
-- Author: Antigravity
...
BEGIN
    ...
    -- OPTIMIZED: Append product details to narration for sales
    -- FIX: Check both v.invoice_id AND le.reference_id (if type is sale)
    SELECT jsonb_agg(t) INTO v_rows FROM (
        SELECT 
            ...
            (SELECT string_agg(i.name, ', ') FROM sale_items si JOIN lots l ON si.lot_id = l.id JOIN items i ON l.item_id = i.id WHERE si.sale_id = le.reference_id) as debug_prod_name,
