-- Migration: Drop Phantom TIMESTAMPTZ function and Ensure Correct DATE function
-- Date: 2026-02-19
-- Author: Antigravity

-- 1. Drop the TIMESTAMPTZ version explicitly to remove ambiguity
DROP FUNCTION IF EXISTS get_ledger_statement(UUID, UUID, TIMESTAMPTZ, TIMESTAMPTZ);

-- 2. Drop the DATE version explicitly too, just to be clean
DROP FUNCTION IF EXISTS get_ledger_statement(UUID, UUID, DATE, DATE);


-- 3. Re-create the function with DATE arguments (This is the one we want)
