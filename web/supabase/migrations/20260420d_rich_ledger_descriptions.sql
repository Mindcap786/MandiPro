-- ============================================================
-- EXACT LEDGER DESCRIPTIONS FORMAT (As requested)
-- ============================================================
...
CREATE OR REPLACE FUNCTION mandi.confirm_sale_transaction(
    p_organization_id   uuid, p_buyer_id uuid, p_sale_date date, p_payment_mode text, p_total_amount numeric,
    p_items jsonb, p_market_fee numeric DEFAULT 0, p_nirashrit numeric DEFAULT 0, p_misc_fee numeric DEFAULT 0,
    p_loading_charges numeric DEFAULT 0, p_unloading_charges numeric DEFAULT 0, p_other_expenses numeric DEFAULT 0,
    p_amount_received numeric DEFAULT NULL, p_idempotency_key text DEFAULT NULL, p_due_date date DEFAULT NULL,
    p_bank_account_id uuid DEFAULT NULL, p_cheque_no text DEFAULT NULL, p_cheque_date date DEFAULT NULL,
    p_cheque_status boolean DEFAULT false, p_bank_name text DEFAULT NULL, p_cgst_amount numeric DEFAULT 0,
    p_sgst_amount numeric DEFAULT 0, p_igst_amount numeric DEFAULT 0, p_gst_total numeric DEFAULT 0,
    p_discount_percent numeric DEFAULT 0, p_discount_amount numeric DEFAULT 0, p_place_of_supply text DEFAULT NULL,
    p_buyer_gstin text DEFAULT NULL, p_is_igst boolean DEFAULT false
) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'mandi', 'public' AS $$
DECLARE
    v_sale_id UUID; v_bill_no BIGINT; v_contact_bill_no BIGINT; v_voucher_id UUID; v_receipt_voucher_id UUID; v_item JSONB;
    v_qty NUMERIC; v_rate NUMERIC; v_total_inc_tax NUMERIC; v_ar_acc_id UUID; v_sales_revenue_acc_id UUID; v_cash_acc_id UUID; v_bank_acc_id UUID;
    v_cheques_transit_acc_id UUID; v_payment_acc_id UUID; v_mode_lower TEXT := LOWER(COALESCE(p_payment_mode, ''));
    v_payment_status TEXT; v_received NUMERIC := 0; v_rec RECORD;
    v_item_details TEXT := ''; v_temp_lot_no TEXT; v_temp_item_name TEXT; v_charges_total NUMERIC; 
    v_sale_narration TEXT; v_receipt_narration TEXT;
BEGIN
    IF p_idempotency_key IS NOT NULL THEN
        FOR v_rec IN (SELECT id, bill_no, contact_bill_no, payment_status, amount_received FROM mandi.sales WHERE idempotency_key = p_idempotency_key LIMIT 1) LOOP
            RETURN jsonb_build_object('success', true, 'sale_id', v_rec.id, 'bill_no', v_rec.bill_no, 'contact_bill_no', v_rec.contact_bill_no, 'payment_status', v_rec.payment_status, 'amount_received', v_rec.amount_received, 'message', 'Idempotent request ignored');
        END LOOP;
    END IF;
    ...
    IF p_bank_account_id IS NOT NULL THEN v_bank_acc_id := p_bank_account_id; ELSE v_bank_acc_id := (SELECT id FROM mandi.accounts WHERE organization_id = p_organization_id AND (code = '1002' OR account_sub_type = 'bank' OR name ILIKE 'Bank%') ORDER BY (code = '1002') DESC LIMIT 1); END IF;
    ...
    FOR v_rec IN (
        WITH new_sale AS (
            INSERT INTO mandi.sales (
                ...
                p_organization_id, p_buyer_id, p_sale_date, p_total_amount, v_total_inc_tax, p_payment_mode, v_payment_status, v_received,
    ...
    FOR v_item IN (SELECT value FROM jsonb_array_elements(p_items)) LOOP
        ...
        IF v_qty > 0 THEN
            INSERT INTO mandi.sale_items (sale_id, lot_id, qty, rate, amount, organization_id) VALUES (v_sale_id, CASE WHEN (v_item->>'lot_id') IS NOT NULL THEN (v_item->>'lot_id')::UUID ELSE NULL END, v_qty, v_rate, ROUND(v_qty * v_rate, 2), p_organization_id);
            IF (v_item->>'lot_id') IS NOT NULL THEN UPDATE mandi.lots SET current_qty = ROUND(COALESCE(current_qty,0) - v_qty, 3) WHERE id = (v_item->>'lot_id')::UUID; END IF;
        END IF;
    END LOOP;
    
    -- Format Exact Sale Narration
    ...
    v_sale_narration := 'Sale Invoice #' || v_bill_no;
    IF LENGTH(TRIM(v_item_details)) > 0 THEN v_sale_narration := v_sale_narration || ' | ' || TRIM(v_item_details); END IF;
    IF v_charges_total > 0 THEN v_sale_narration := v_sale_narration || ' | Extra Charges: ₹' || v_charges_total; END IF;

    FOR v_rec IN (WITH new_voucher AS (INSERT INTO mandi.vouchers (organization_id, date, type, voucher_no, amount, narration, invoice_id) VALUES (p_organization_id, p_sale_date, 'sale', (SELECT COALESCE(MAX(voucher_no),0)+1 FROM mandi.vouchers WHERE organization_id = p_organization_id AND type = 'sale'), v_total_inc_tax, v_sale_narration, v_sale_id) RETURNING id) SELECT * FROM new_voucher) LOOP v_voucher_id := v_rec.id; END LOOP;
    ...
    -- Format Exact Receipt Narration
    IF v_received > 0 AND v_mode_lower NOT IN ('udhaar','credit') THEN
        ...
        v_receipt_narration := 'Payment on Sale Invoice #' || v_bill_no;

        FOR v_rec IN (WITH new_receipt AS (INSERT INTO mandi.vouchers (organization_id, date, type, voucher_no, amount, narration, invoice_id) VALUES (p_organization_id, p_sale_date, 'receipt', (SELECT COALESCE(MAX(voucher_no),0)+1 FROM mandi.vouchers WHERE organization_id = p_organization_id AND type = 'receipt'), v_received, v_receipt_narration, v_sale_id) RETURNING id) SELECT * FROM new_receipt) LOOP v_receipt_voucher_id := v_rec.id; END LOOP;
...
DECLARE
    ...
    v_purchase_acc_id UUID; v_expense_recovery_acc_id UUID; v_cash_acc_id UUID;
    v_total_commission NUMERIC := 0; v_total_inventory NUMERIC := 0; v_total_transport NUMERIC := 0;
    v_main_voucher_id UUID; v_gross_bill NUMERIC; v_total_advance_cleared NUMERIC := 0; v_final_status TEXT := 'pending';
    v_adv RECORD; v_contra_acc UUID; 
    v_lot_details TEXT := ''; v_purchase_narration TEXT; v_payment_narration TEXT;
    v_rec RECORD;
BEGIN
    FOR v_arrival IN (SELECT a.*, c.name AS party_name FROM mandi.arrivals a LEFT JOIN mandi.contacts c ON a.party_id = c.id WHERE a.id = p_arrival_id) LOOP
    ...
    IF v_org_id IS NULL THEN RETURN jsonb_build_object('success', false, 'error', 'Arrival not found'); END IF;
    ...
    v_purchase_acc_id := (SELECT id FROM mandi.accounts WHERE organization_id = v_org_id AND code = '5001' LIMIT 1);
    ...
    v_purchase_narration := 'Purchase Invoice #' || v_reference_no;
    IF LENGTH(TRIM(v_lot_details)) > 0 THEN v_purchase_narration := v_purchase_narration || ' | ' || TRIM(v_lot_details); END IF;
    IF v_total_transport > 0 THEN v_purchase_narration := v_purchase_narration || ' | Extra Charges: ₹' || v_total_transport; END IF;

    FOR v_rec IN (
        WITH new_voucher AS (
            INSERT INTO mandi.vouchers (organization_id, date, type, voucher_no, narration, amount, party_id, arrival_id) 
            VALUES (v_org_id, v_arrival_date, 'purchase', (SELECT COALESCE(MAX(voucher_no), 0) + 1 FROM mandi.vouchers WHERE organization_id = v_org_id AND type = 'purchase'), v_purchase_narration, v_gross_bill, v_party_id, p_arrival_id) 
            RETURNING id
        ) SELECT * FROM new_voucher
    ) LOOP v_main_voucher_id := v_rec.id; END LOOP;

    INSERT INTO mandi.ledger_entries (organization_id, voucher_id, account_id, debit, credit, entry_date, description, transaction_type, reference_id) VALUES (v_org_id, v_main_voucher_id, v_purchase_acc_id, v_gross_bill, 0, v_arrival_date, v_purchase_narration, 'purchase', p_arrival_id);
    
    IF v_party_id IS NOT NULL THEN
        INSERT INTO mandi.ledger_entries (organization_id, voucher_id, contact_id, debit, credit, entry_date, description, transaction_type, reference_id) VALUES (v_org_id, v_main_voucher_id, v_party_id, 0, v_gross_bill, v_arrival_date, v_purchase_narration, 'purchase', p_arrival_id);
        IF v_total_transport > 0 THEN
            INSERT INTO mandi.ledger_entries(organization_id, voucher_id, contact_id, debit, credit, entry_date, description, transaction_type, reference_id) VALUES (v_org_id, v_main_voucher_id, v_party_id, v_total_transport, 0, v_arrival_date, 'Extra Charges for Invoice #' || v_reference_no, 'purchase', p_arrival_id);
            INSERT INTO mandi.ledger_entries(organization_id, voucher_id, account_id, debit, credit, entry_date, description, transaction_type, reference_id) VALUES (v_org_id, v_main_voucher_id, v_expense_recovery_acc_id, 0, v_total_transport, v_arrival_date, 'Extra Charges for Invoice #' || v_reference_no, 'purchase', p_arrival_id);
        END IF;
    END IF;

    v_payment_narration := 'Payment on Purchase Invoice #' || v_reference_no;
            ...
            IF v_party_id IS NOT NULL THEN
                ...
                FOR v_rec IN (
                    WITH new_payment AS (
                        ...
                        VALUES (v_org_id, v_arrival_date, 'payment', (SELECT COALESCE(MAX(voucher_no), 0) + 1 FROM mandi.vouchers WHERE organization_id = v_org_id AND type = 'payment'), v_party_id, v_adv.total_adv, p_arrival_id, v_payment_narration)
                        RETURNING id
                    ) SELECT * FROM new_payment
                ) LOOP NULL; END LOOP;

                INSERT INTO mandi.ledger_entries(organization_id, voucher_id, contact_id, debit, credit, entry_date, description, transaction_type, reference_id) VALUES (v_org_id, v_main_voucher_id, v_party_id, v_adv.total_adv, 0, v_arrival_date, v_payment_narration, 'payment', p_arrival_id);
                INSERT INTO mandi.ledger_entries(organization_id, voucher_id, account_id, debit, credit, entry_date, description, transaction_type, reference_id) VALUES (v_org_id, v_main_voucher_id, v_contra_acc, 0, v_adv.total_adv, v_arrival_date, v_payment_narration, 'payment', p_arrival_id);
            END IF;
        END IF;
    END LOOP;

    IF v_total_advance_cleared >= (v_gross_bill - v_total_transport - 0.01) THEN v_final_status := 'paid'; ELSIF v_total_advance_cleared > 0 THEN v_final_status := 'partial'; ELSE v_final_status := 'pending'; END IF;
    UPDATE mandi.arrivals SET amount_deducted = v_total_advance_cleared, payment_status = v_final_status WHERE id = p_arrival_id;
    RETURN jsonb_build_object('success', true, 'purchase_voucher_id', v_main_voucher_id);
END;
$function$;
