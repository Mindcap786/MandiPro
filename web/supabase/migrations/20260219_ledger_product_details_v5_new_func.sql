-- Migration: Create get_ledger_statement_v2
-- Date: 2026-02-19
-- Author: Antigravity

CREATE OR REPLACE FUNCTION get_ledger_statement_v2(
...
BEGIN
    ...
    SELECT jsonb_agg(t) INTO v_rows FROM (
        SELECT 
            ...
            -- DEBUG FIELDS
            v.id as debug_voucher_id,
            le.reference_id as debug_ref_id,
            v.invoice_id as debug_inv_id,
            (SELECT string_agg(i.name, ', ') FROM sale_items si JOIN lots l ON si.lot_id = l.id JOIN items i ON l.item_id = i.id WHERE si.sale_id = le.reference_id) as debug_prod_name_ref,
             (SELECT string_agg(i.name, ', ') FROM sale_items si JOIN lots l ON si.lot_id = l.id JOIN items i ON l.item_id = i.id WHERE si.sale_id = v.invoice_id) as debug_prod_name_inv,
            ...
            CASE 
                ...
                -- Case 2: Linked via Ledger Entry Reference
