...
-- Migration: 20260416_world_class_sale_transaction.sql
-- ============================================================
...
BEGIN
    ...
    INSERT INTO mandi.ledger_entries (organization_id, voucher_id, contact_id, debit, credit, entry_date, transaction_type)
    VALUES (p_organization_id, v_voucher_id, p_buyer_id, v_total_receivable, 0, p_sale_date, 'sale');

    -- Entry 2: Credit Sales (Net Revenue)
    INSERT INTO mandi.ledger_entries (organization_id, voucher_id, account_id, debit, credit, entry_date, transaction_type)
    VALUES (p_organization_id, v_voucher_id, v_sales_acc, 0, p_total_amount - p_discount_amount, p_sale_date, 'sale');

    -- Entry 3: Credit Fees
    IF (p_market_fee + p_nirashrit + p_misc_fee) > 0 THEN
        INSERT INTO mandi.ledger_entries (organization_id, voucher_id, account_id, debit, credit, entry_date, transaction_type)
        VALUES (p_organization_id, v_voucher_id, v_market_fee_acc, 0, p_market_fee + p_nirashrit + p_misc_fee, p_sale_date, 'sale_fee');
    END IF;

    -- Entry 4: Credit Taxes
    IF p_cgst_amount > 0 THEN
        INSERT INTO mandi.ledger_entries (organization_id, voucher_id, account_id, debit, credit, entry_date, transaction_type)
        VALUES (p_organization_id, v_voucher_id, v_cgst_acc, 0, p_cgst_amount, p_sale_date, 'gst');
    END IF;
    IF p_sgst_amount > 0 THEN
        INSERT INTO mandi.ledger_entries (organization_id, voucher_id, account_id, debit, credit, entry_date, transaction_type)
        VALUES (p_organization_id, v_voucher_id, v_sgst_acc, 0, p_sgst_amount, p_sale_date, 'gst');
    END IF;
    IF p_igst_amount > 0 THEN
        INSERT INTO mandi.ledger_entries (organization_id, voucher_id, account_id, debit, credit, entry_date, transaction_type)
        VALUES (p_organization_id, v_voucher_id, v_igst_acc, 0, p_igst_amount, p_sale_date, 'gst');
    END IF;

    -- Entry 5: Credit Expenses (Loading etc)
    IF (p_loading_charges + p_unloading_charges + p_other_expenses) > 0 THEN
        INSERT INTO mandi.ledger_entries (organization_id, voucher_id, account_id, debit, credit, entry_date, transaction_type)
        VALUES (p_organization_id, v_voucher_id, v_sales_acc, 0, p_loading_charges + p_unloading_charges + p_other_expenses, p_sale_date, 'sale_expense');
    END IF;
