-- ================================================================
-- CRITICAL FIX: confirm_sale_transaction — column "system_type" does not exist
-- Date: 2026-04-20
--
-- ROOT CAUSE:
-- The active confirm_sale_transaction RPC looked up accounts using:
--     WHERE system_type = 'accounts_receivable'
--     WHERE system_type = 'sales_revenue'
--     WHERE system_type = 'cash'
-- But mandi.accounts has NO system_type column.
-- Actual columns are: code, account_sub_type, name
-- → Every sale creation failed with "column system_type does not exist"
--
-- FIX:
-- Replace all system_type lookups with code/account_sub_type/ILIKE fallbacks:
--     AR:    code='1100' OR account_sub_type='accounts_receivable'
--     Sales: code='4001' OR name ILIKE 'Sales%'
--     Cash:  code='1001' OR account_sub_type='cash'
--     Bank:  code='1002' OR account_sub_type='bank'
--
-- Also ensures:
-- - amount_received written to mandi.sales for Tier 2 of get_invoice_balance
-- - Receipt voucher created with invoice_id set for Tier 1 of get_invoice_balance
-- - All 3 scenarios: Full, Partial, Udhaar/Credit work correctly
-- - Stock deducted on sale
-- - DOES NOT TOUCH: purchase, arrivals, other ledger paths
-- ================================================================
...
DECLARE
    ...
    v_mode_lower     TEXT    := LOWER(COALESCE(p_payment_mode, 'pending'));
...
BEGIN
    ...
    -- Account lookups using code / account_sub_type (NOT system_type)
    ...
    WHERE organization_id = p_organization_id
      AND (code = '4001' OR name ILIKE 'Sales%')
    ...
    WHERE organization_id = p_organization_id
      AND (code = '1003' OR account_sub_type IN ('cheque','cheques_in_transit') OR name ILIKE '%Cheque%')
    ORDER BY (account_sub_type = 'cheques_in_transit') DESC LIMIT 1;
    ...
    IF v_cash_acc_id IS NULL THEN
        RAISE EXCEPTION 'Cash account (code 1001) not found for org %', p_organization_id;
    END IF;

    -- Invoice total
    v_total_inc_tax := ROUND((
        COALESCE(p_total_amount,0) + COALESCE(p_market_fee,0) + COALESCE(p_nirashrit,0) +
        COALESCE(p_misc_fee,0) + COALESCE(p_loading_charges,0) + COALESCE(p_unloading_charges,0) +
        COALESCE(p_other_expenses,0) + COALESCE(p_gst_total,0) - COALESCE(p_discount_amount,0)
    )::NUMERIC, 2);
    ...
    -- Insert sale items + stock deduction
    FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
        ...
        IF v_qty > 0 THEN
            ...
            IF (v_item.value->>'lot_id') IS NOT NULL THEN
    ...
    INSERT INTO mandi.vouchers (
        ...
        'Sale Invoice #' || v_bill_no || ' — ' || COALESCE(p_payment_mode,''),
        v_sale_id, p_buyer_id, p_payment_mode, v_sale_id
    ...
    (p_organization_id, v_voucher_id, v_ar_acc_id, p_buyer_id,
     v_total_inc_tax, 0, p_sale_date, 'Sale Bill #' || v_bill_no, 'sale', v_sale_id),
    (p_organization_id, v_voucher_id, v_sales_revenue_acc_id, NULL,
     0, v_total_inc_tax, p_sale_date, 'Sales Revenue — Bill #' || v_bill_no, 'sale', v_sale_id);

    -- Receipt voucher (only if payment received at sale time)
