-- Drop conflicting signatures to resolve ambiguity
DROP FUNCTION IF EXISTS public.confirm_sale_transaction(uuid, uuid, text, text, numeric, jsonb, numeric, numeric, uuid);
DROP FUNCTION IF EXISTS public.confirm_sale_transaction(uuid, uuid, timestamp with time zone, text, numeric, jsonb, numeric, numeric, uuid);

-- Re-assert the correct function definition to be sure
