-- ============================================================
-- FIX LEDGER STATEMENT GROUPING FOR SINGLE TRANSACTION UI
-- Migration: 20260419_statement_group_fix.sql
-- ============================================================
        ...
        SELECT
            ...
            CASE
                WHEN MAX(v_type) = 'sales' OR MAX(raw_description) ILIKE 'Invoice #%' THEN
                    CASE WHEN SUM(debit) > 0 THEN 'SALE (CREDIT)' ELSE 'SALE (FULL/PARTIAL)' END
            ...
            CASE
                ...
                WHEN MAX(transaction_type) LIKE 'sale%' THEN
                    MAX(COALESCE(v_narration, raw_description, 'Sale Transaction'))
        ...
        SELECT
            *,
            SUM(COALESCE(debit,0) - COALESCE(credit,0))
                OVER (ORDER BY entry_date ASC, sort_id ASC)    AS running_diff
        FROM grouped_data
    )
    SELECT jsonb_agg(
        jsonb_build_object(
