-- Migration: Fix Product Details Logic (Remove 'Mix' fallback)
-- Date: 2026-02-19
-- Author: Antigravity
...
BEGIN
    ...
    -- FIX: Removed 'Mix' fallback. If string_agg is NULL, the concatenation yields NULL, so we coalesce to original narration.
    SELECT jsonb_agg(t) INTO v_rows FROM (
        SELECT 
            ...
            -- Append product names if it's a sale with valid invoice_id
            CASE 
