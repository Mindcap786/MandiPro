...
export default function SaasBillingPage() {
    ...
    const fetchAll = async () => {
        setLoading(true);
        try {
            const [subRes, plansRes, invoicesRes, usageRes, storageRes] = await Promise.all([
            ...
            if (plansRes.error) {
                console.error("Billing: Error fetching plans:", plansRes.error);
            }
            if (subRes.error && subRes.error.code !== 'PGRST116') console.error("Billing: Error fetching subscription:", subRes.error);

            const plansList = plansRes.data || [];

            let subData = subRes.data;
            if (subData) {
                subData.plan = plansList.find((p: any) => p.id === subData.plan_id);
            }

            // Fallback for new/trial orgs without a core.subscriptions record
            if (!subData || !subData.plan) {
                const orgTier = profile?.organization?.subscription_tier?.toLowerCase() || 'basic';
                const matchedPlan = plansList.find((p: any) => p.name?.toLowerCase() === orgTier)
                    || plansList.find((p: any) => p.name?.toLowerCase() === 'basic')
                    || plansList[0];
                subData = {
                    plan: matchedPlan,
                    status: profile?.organization?.status || 'trial',
            ...
            const metrics = usageRes.data || [];
            setUsage({
                lots: metrics.reduce((s: number, m: any) => s + (m.lots_created || 0), 0),
                sales: metrics.reduce((s: number, m: any) => s + (m.sales_created || 0), 0),
                payments: metrics.reduce((s: number, m: any) => s + (m.payments_created || 0), 0),
                storageGb: storageRes.data || 0,
            });
            setLoading(false);
        } catch (err) {
            console.error("Billing: Unexpected error in fetchAll:", err);
            setLoading(false);
        }
    };
