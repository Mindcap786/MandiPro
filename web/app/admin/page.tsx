...
export default function AdminDashboard() {
    const [metrics, setMetrics] = useState<any>({
        total_mandis: 0, active_mandis: 0, negative_stock_count: 0,
        negative_ledger_count: 0, total_gmv: 0, churn_risk_count: 0,
        mrr: 0, arr: 0, health_score: 100, system_status: 'checking'
    });
    const [loading, setLoading] = useState(true);
    const [lastUpdated, setLastUpdated] = useState<Date>(new Date());
    const [recentTenants, setRecentTenants] = useState<any[]>([]);
    const [monitoring, setMonitoring] = useState<any>({ platform_status: 'checking', critical_alerts: [], warning_alerts: [] });
    ...
    const fetchAll = async () => {
        ...
        // 1. Trigger Platform Lifecycle Sync (SaaS Automation)
        ...
        const [metricsRes, tenantsRes, monitoringRes] = await Promise.allSettled([
            supabase.rpc('get_global_risk_metrics'),
        ...
        if (metricsRes.status === 'fulfilled' && metricsRes.value.data) {
            ...
            // Sync local monitoring state with DB metrics
            setMonitoring((prev: any) => ({
                ...prev,
                platform_status: metricsRes.value.data.system_status,
                critical_count: metricsRes.value.data.critical_alerts_count
            }));
        }
