/**
 * Sales Tax & Totals Engine — 1:1 Mirror of Web lib/sales-tax.ts
 */
...
export function calculateSaleItemTaxBreakdown({
    ...
    const cgstAmount = Math.round(cgstRaw);
    const sgstAmount = Math.round(sgstRaw);
    const igstAmount = Math.round(igstRaw);
...
export function calculateSaleTotals({
    ...
    const marketFee = Math.round(taxableSubTotal * (toNumber(taxSettings.market_fee_percent) / 100));
    const nirashrit = Math.round(taxableSubTotal * (toNumber(taxSettings.nirashrit_percent) / 100));
    const miscFee = Math.round(taxableSubTotal * (toNumber(taxSettings.misc_fee_percent) / 100));
    ...
    const taxBreakdowns = items.map((item) =>
        calculateSaleItemTaxBreakdown({
            amount: toNumber(item.amount) * discountRatio,
    ...
    const extraCharges = toNumber(loadingCharges) + toNumber(unloadingCharges) + toNumber(otherExpenses);
    const grandTotal = Math.round(taxableSubTotal + marketFee + nirashrit + miscFee + gstTotal + extraCharges);
