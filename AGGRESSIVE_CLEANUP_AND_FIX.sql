-- ================================================================
-- AGGRESSIVE CLEANUP: Remove ALL confirm_sale_transaction versions
...
-- Expected: Multiple rows (this is the problem!)


-- ================================================================
-- Step 2: Drop ALL triggers that might reference this function
-- ================================================================
DO $$
DECLARE
    trig RECORD;
BEGIN
    FOR trig IN (
        ...
        AND (action_statement ILIKE '%confirm_sale%' 
             OR action_statement ILIKE '%sale%')
    )
    LOOP
        BEGIN
            EXECUTE 'DROP TRIGGER IF EXISTS ' || quote_ident(trig.trigger_name) || 
                    ' ON ' || quote_ident(trig.event_object_table) || ' CASCADE';
...
-- ================================================================
-- Step 3: FORCE DROP each function version individually
-- ================================================================
DO $$
DECLARE
    func RECORD;
...
BEGIN
    ...
    LOOP
        BEGIN
            drop_sql := 'DROP FUNCTION IF EXISTS ' || func.sig || ' CASCADE';
            EXECUTE drop_sql;
            RAISE NOTICE 'Dropped: %', func.sig;
        EXCEPTION WHEN OTHERS THEN
            RAISE NOTICE 'Error dropping %: %', func.sig, SQLERRM;
        END;
    END LOOP;
END $$;
...
-- ================================================================
-- Step 4: Verify ALL functions are deleted
-- ================================================================
SELECT 
...
-- Expected: 0 (zero functions remaining)


-- ================================================================
-- Step 5: NOW create the new clean function
-- ================================================================
...
BEGIN
    ...
    INSERT INTO mandi.ledger_entries (
        ...
        'Sale Invoice #' || v_bill_no, 'purchase', v_bill_no::text, v_sale_id
    );
    ...
    IF v_sales_revenue_acc_id IS NOT NULL THEN
        INSERT INTO mandi.ledger_entries (
            ...
            p_sale_date, 'Sales Revenue #' || v_bill_no, 'purchase', v_bill_no::text, v_sale_id
        );
    END IF;
...
-- Expected: function_count = 1 and status = 'SUCCESS ✅'
