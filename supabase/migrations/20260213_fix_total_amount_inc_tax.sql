-- Fix total_amount_inc_tax to include buyer expenses
...
-- Recreate with total_amount_inc_tax calculation
...
BEGIN
    ...
    -- Calculate total payable including all fees and buyer expenses
    ...
    -- 2. Insert Sale Record with buyer expenses and total_amount_inc_tax
    INSERT INTO sales (
        organization_id, buyer_id, sale_date, payment_mode, total_amount, total_amount_inc_tax, bill_no, 
        market_fee, nirashrit, misc_fee, loading_charges, unloading_charges, other_expenses,
        payment_status, created_at, idempotency_key
    ) VALUES (
        p_organization_id, p_buyer_id, p_sale_date, p_payment_mode, p_total_amount, v_total_payable, v_bill_no, 
    ...
    IF v_sales_account_id IS NOT NULL THEN
        INSERT INTO ledger_entries (
            ...
            p_organization_id, v_voucher_id, v_sales_account_id, 0, v_total_payable -- All revenue including buyer expenses
        );
    END IF;
