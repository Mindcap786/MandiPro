-- Fix: post_arrival_ledger fails when arrival has no party (direct purchase without supplier)
-- Root cause: INNER JOIN mandi.contacts causes "Arrival not found" when party_id IS NULL
-- Fix: Use LEFT JOIN and guard against NULL party_id in voucher/ledger inserts
...
BEGIN
    ...
    -- FIX: Use LEFT JOIN so arrivals without a party (direct purchases) are still found
    ...
    v_party_id := v_arrival.party_id; -- May be NULL for party-less direct purchases
    ...
    INSERT INTO mandi.vouchers (
        ...
        v_party_id, p_arrival_id  -- party_id may be NULL; that's acceptable for direct purchases
    ...
    IF v_party_id IS NOT NULL THEN
        ...
        -- Transport Recovery (only if party exists to debit)
        ...
        -- Commission Income (only if party exists)
        ...
        END IF;
    ELSE
        -- No party: still record transport and commission against the account (no party ledger entry)
        IF v_total_transport > 0 THEN
            INSERT INTO mandi.ledger_entries (organization_id, voucher_id, account_id, debit, credit, entry_date, description, transaction_type, reference_id, products)
