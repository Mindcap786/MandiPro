-- Add products extraction to ledger statement
...
BEGIN
    ...
    WITH ranked_tx AS (
        SELECT 
            ...
            -- Extract Product Listing directly to the UI
            CASE
                WHEN v.type = 'sales' THEN (
                    SELECT jsonb_agg(
                        jsonb_build_object(
                    ...
                    JOIN items i ON l.item_id = i.id
                    WHERE s.organization_id = p_organization_id
                      AND s.bill_no = v.voucher_no
                )
                WHEN le.description LIKE '%LOT-%' OR COALESCE(v.narration, '') LIKE '%LOT-%' THEN (
                    SELECT jsonb_agg(
                        jsonb_build_object(
                            'name', i.name,
                            'qty', l.initial_qty,
                    ...
                    WHERE l.organization_id = p_organization_id
                      AND (
                          le.description LIKE '%' || l.lot_code || '%'
                          OR 
                          COALESCE(v.narration, '') LIKE '%' || l.lot_code || '%'
                      )
                )
                ELSE NULL
            END as products,
