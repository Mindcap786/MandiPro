-- ============================================================
-- CRITICAL FIX: Account Code Type Mismatch in post_arrival_ledger
-- ============================================================
-- Root Cause: Migration 20260421 introduced numeric literals for
-- code comparisons, but the 'code' column is TEXT type.
-- This causes: "operator does not exist: text = integer"
--
-- Impact: "Commission arrival logged successfully" but then fails
-- with "Ledger Sync Warning: operator does not exist: text = integer"
-- when the function tries to fetch account codes.
--
-- Why it repeats: Every arrival log triggers post_arrival_ledger RPC
-- which attempts to compare text column with numeric literals.
--
-- Permanent Fix: Update post_arrival_ledger function to use string
-- literals for ALL code comparisons, ensuring type safety.
-- ============================================================
...
BEGIN
    ...
    -- 1. Get accounts - FIXED: Use string literals for code comparisons to match TEXT column type
...
-- ============================================================
-- Safety Check: Ensure this function doesn't break existing data
-- The changes are purely type-safety fixes - no logic changed
-- ============================================================
COMMENT ON FUNCTION mandi.post_arrival_ledger(uuid) IS
'Posts arrival ledger entries with corrected type-safe account code comparisons.
Version: 20260414 - Fixed text/integer type mismatch for code column.
RETURNS: { success: boolean, arrival_id: uuid, status: text, message: text }';
