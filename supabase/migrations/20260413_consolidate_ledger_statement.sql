-- ─── ARCHITECTURAL CLEANUP & HYBRID LEDGER RPC ───────────
-- 1. Remove legacy conflict from public schema
DROP FUNCTION IF EXISTS public.get_ledger_statement(uuid, uuid, timestamptz, timestamptz);
DROP FUNCTION IF EXISTS public.get_ledger_statement(uuid, date, date, uuid, varchar);

-- 2. Implement Hybrid mandi.get_ledger_statement
-- We support both p_from_date/p_to_date AND p_start_date/p_end_date for 100% compatibility.
...
DECLARE
    ...
    v_to DATE;
BEGIN
    -- 1. Resolve Parameters (Hybrid Logic)
    ...
    -- 4. Transactions with Itemized Injection
    WITH ledger_base AS (
        SELECT
            le.id,
            le.entry_date,
...
-- Ensure mandi schema is in search path for the API role if not already
-- (Optional but recommended for senior ERP arch)
-- ALTER ROLE authenticator SET search_path = mandi, public;
