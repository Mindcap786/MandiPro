-- CRITICAL FIX: Remove conflicting old mandi.confirm_sale_transaction and create clean version
-- Problem: Database has 9-parameter version, client sends 25-parameter version
-- Solution: Drop all versions and create single correct implementation

BEGIN;

-- 1. Drop the OLD broken version
DROP FUNCTION IF EXISTS mandi.confirm_sale_transaction(uuid, uuid, date, numeric, text, numeric, jsonb, numeric, uuid) CASCADE;
DROP FUNCTION IF EXISTS mandi.confirm_sale_transaction(uuid, uuid, date, text, numeric, jsonb, numeric, uuid, numeric, numeric, numeric, numeric, numeric, numeric, numeric, uuid, date, uuid, text, date, boolean, text, numeric, numeric, numeric, numeric, numeric, numeric, text, text, boolean) CASCADE;

-- 2. Create SINGLE, CLEAN, CORRECT version with all 25 parameters
...
BEGIN
    ...
    INSERT INTO mandi.sales (
        organization_id, buyer_id, sale_date, total_amount, total_amount_inc_tax, payment_mode, payment_status,
        market_fee, nirashrit, misc_fee, loading_charges, unloading_charges, other_expenses, due_date,
        cheque_no, cheque_date, bank_name, bank_account_id, cgst_amount, sgst_amount, igst_amount, gst_total,
        discount_percent, discount_amount, place_of_supply, buyer_gstin, idempotency_key
    )
    VALUES (
        p_organization_id, p_buyer_id, p_sale_date, p_total_amount, v_total_inc_tax, p_payment_mode, v_payment_status,
        p_market_fee, p_nirashrit, p_misc_fee, p_loading_charges, p_unloading_charges, p_other_expenses, p_due_date,
        p_cheque_no, p_cheque_date, p_bank_name, p_bank_account_id, p_cgst_amount, p_sgst_amount, p_igst_amount, p_gst_total,
    ...
    VALUES (p_organization_id, p_sale_date, 'sale', (SELECT COALESCE(MAX(voucher_no), 0) + 1 FROM mandi.vouchers WHERE organization_id = p_organization_id AND type = 'sale'), v_total_inc_tax, 'Sale #' || v_bill_no, v_sale_id, p_buyer_id, p_payment_mode)
    ...
    RETURN jsonb_build_object('success', true, 'payment_status', v_payment_status, 'sale_id', v_sale_id, 'bill_no', v_bill_no);
END;
$function$;
