-- CLEANUP ARRIVALS FIELDS: REMOVE VARIETY AND GRADE (v5.23)
-- Migration: 20260421000007_cleanup_arrivals_fields.sql

-- 1. Remove from existing field configurations
DELETE FROM mandi.field_configs 
WHERE module_id IN ('arrivals_direct', 'arrivals_farmer', 'arrivals_supplier')
AND field_key IN ('variety', 'grade');

-- 2. Update the seed function to exclude these fields in the future
